...
namespace TvProgViewer.Plugin.Payments.PayPalViewer.Services
{
    ...
    public class OnboardingHttpClient
    {
        ...
        {
            //configure client
            httpClient.BaseAddress = new Uri(PayPalViewerDefaults.Onboarding.ServiceUrl);
            httpClient.Timeout = TimeSpan.FromSeconds(PayPalViewerDefaults.Onboarding.RequestTimeout);
            httpClient.DefaultRequestHeaders.Add(HeaderNames.UserAgent, PayPalViewerDefaults.UserAgent);
        ...
        public async Task<TResponse> RequestAsync<TRequest, TResponse>(TRequest request) where TRequest : Request where TResponse : class
        {
            try
            {
                ...
                if (result.Result == ResponseResult.Error)
                    throw new TvProgException($"Onboarding error - {result.Error}");
