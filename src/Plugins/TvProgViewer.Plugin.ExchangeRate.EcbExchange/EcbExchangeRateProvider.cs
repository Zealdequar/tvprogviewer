...
namespace TvProgViewer.Plugin.ExchangeRate.EcbExchange
{
    public class EcbExchangeRateProvider : BasePlugin, IExchangeRateProvider
    {
        ...
        public async Task<IList<Core.Domain.Directory.ExchangeRate>> GetCurrencyLiveRatesAsync(string exchangeRateCurrencyCode)
        {
            ...
            try
            {
                var httpClient = _httpClientFactory.CreateClient(TvProgHttpDefaults.DefaultHttpClient);
            ...
            if (exchangeRateCurrency == null)
                throw new TvProgException(await _localizationService.GetResourceAsync("Plugins.ExchangeRate.EcbExchange.Error"));
