...
namespace TvProgViewer.Plugin.Payments.CyberSource.Services
{
    ...
    public class CyberSourceService
    {
        ...
        /// </returns>
        private async Task<PtsV2PaymentsPost201Response> AuthorizeAsync(TvProgViewer.Services.Payments.ProcessPaymentRequest processPaymentRequest,
            bool capture, bool saveCardOnFile)
        {
            ...
            var orderInformation = await PrepareOrderInformationAsync(processPaymentRequest.UserId, processPaymentRequest.OrderTotal);
            ...
            if (saveCardOnFile)
            {
                ...
                {
                    CyberSourceDefaults.UserActionTokenTypeName,
        ...
        /// </summary>
        /// <param name="processPaymentRequest">Process payment request</param>
        /// <param name="userTokenId">Cybersource user token id</param>
        /// <param name="capture">Authorization with capture (Sale)</param>
        ...
        /// The task result contains the authorized response
        /// </returns>
        private async Task<PtsV2PaymentsPost201Response> AuthorizeWithTokenAsync(TvProgViewer.Services.Payments.ProcessPaymentRequest processPaymentRequest,
            int userTokenId, bool capture, string authenticationTransactionId)
        {
            ...
            {
                throw new TvProgException("Payer authentication failed");
            }

            //ensure that tokenization is enabled
            if (!_cyberSourceSettings.TokenizationEnabled)
                throw new TvProgException("Tokenization not enabled");

            var userToken = await _userTokenService.GetByIdAsync(userTokenId)
                ?? throw new TvProgException("Saved card data not found");

            if (userToken.UserId != processPaymentRequest.UserId)
                throw new TvProgException("Saved card data is incorrect");

            //ensure that token is valid
            if (string.IsNullOrEmpty(userToken.SubscriptionId))
                throw new TvProgException("Invalid token");
            ...
            var processingInformation = new Ptsv2paymentsProcessingInformation(Capture: capture,
                AuthorizationOptions: authorizationOptions,
                ActionList: !_cyberSourceSettings.DecisionManagerEnabled ? new() { CyberSourceDefaults.DecisionSkipActionName } : new());
            var paymentInformationCard = new Ptsv2paymentsPaymentInformationCard(ExpirationMonth: userToken.CardExpirationMonth,
                ExpirationYear: userToken.CardExpirationYear);
            var paymentInstrument = new Ptsv2paymentsPaymentInformationPaymentInstrument(Id: userToken.SubscriptionId);
            var paymentInformation = new Ptsv2paymentsPaymentInformation(Card: paymentInformationCard,
        ...
        /// The task result contains the authorized response
        /// </returns>
        private async Task<PtsV2PaymentsPost201Response> AuthorizeWithTransientTokenAsync(TvProgViewer.Services.Payments.ProcessPaymentRequest processPaymentRequest,
            string transientToken, bool capture, bool saveCardOnFile, string authenticationTransactionId)
        {
            ...
            if (_cyberSourceSettings.PaymentConnectionMethod != ConnectionMethodType.FlexMicroForm)
                throw new TvProgException("Flex microform flow not enabled");
            ...
            if (string.IsNullOrEmpty(transientToken))
                throw new TvProgException("Invalid transient token");
        ...
        /// </returns>
        private async Task<Ptsv2paymentsOrderInformation> PrepareOrderInformationAsync(int userId, decimal orderTotal)
        {
            ...
            var billingAddress = await _addressService.GetAddressByIdAsync(user.BillingAddressId ?? 0);
            ...
            var shippingAddress = await _addressService.GetAddressByIdAsync(user.ShippingAddressId ?? 0);
        ...
        /// </returns>
        public async Task<(PtsV2PaymentsPost201Response Result, string Error)> AuthorizeAsync(TvProgViewer.Services.Payments.ProcessPaymentRequest processPaymentRequest,
            bool isNewCard = false, bool saveCardOnFile = false,
            int? userTokenId = null, string transientToken = null, string authenticationTransactionId = null)
        {
        ...
        /// </returns>
        public async Task<(PtsV2PaymentsCapturesPost201Response Result, string Error)> CaptureAsync(TvProgViewer.Services.Payments.CapturePaymentRequest capturePaymentRequest)
        {
            return await HandleFunctionAsync(async () =>
            {
                var currency = (await _currencyService.GetCurrencyByIdAsync(_currencySettings.PrimaryStoreCurrencyId))?.CurrencyCode;
        ...
        /// </returns>
        public async Task<(PtsV2PaymentsVoidsPost201Response Result, string Error)> VoidPaymentAsync(TvProgViewer.Services.Payments.VoidPaymentRequest voidPaymentRequest)
        {
            return await HandleFunctionAsync(async () =>
            {
        ...
        /// The task result contains the voided response; Error message if exists
        /// </returns>
        public async Task<(PtsV2PaymentsVoidsPost201Response Result, string Error)> VoidCaptureAsync(TvProgViewer.Services.Payments.VoidPaymentRequest voidPaymentRequest)
        {
            return await HandleFunctionAsync(async () =>
            {
                var clientReferenceInformation = new Ptsv2paymentsidreversalsClientReferenceInformation(Code: voidPaymentRequest.Order?.OrderGuid.ToString(),
        ...
        /// </returns>
        public async Task<(PtsV2PaymentsRefundPost201Response Result, string Error)> RefundAsync(TvProgViewer.Services.Payments.RefundPaymentRequest refundPaymentRequest)
        {
            return await HandleFunctionAsync(async () =>
            {
                var currency = (await _currencyService.GetCurrencyByIdAsync(_currencySettings.PrimaryStoreCurrencyId))?.CurrencyCode;
        ...
        /// </returns>
        public async Task<(PtsV2CreditsPost201Response Result, string Error)> CreditAsync(TvProgViewer.Services.Payments.RefundPaymentRequest refundPaymentRequest)
        {
            return await HandleFunctionAsync(async () =>
            {
                var currency = (await _currencyService.GetCurrencyByIdAsync(_currencySettings.PrimaryStoreCurrencyId))?.CurrencyCode;
        ...
        /// </summary>
        /// <param name="userToken">Cybersource user token</param>
        /// <param name="transientToken">Flex microform transient token</param>
        /// <returns>
        /// A task that represents the asynchronous operation
        /// The task result contains the setup payer authentication response; Error message if exists
        /// </returns>
        public async Task<(RiskV1AuthenticationSetupsPost201Response Result, string Error)> PayerAuthenticationSetupAsync(CyberSourceUserToken userToken,
            string transientToken)
        {
            return await HandleFunctionAsync(async () =>
            {
                ...
                if (!_cyberSourceSettings.PayerAuthenticationEnabled)
                    throw new TvProgException("Payer authentication not enabled");

                if (string.IsNullOrEmpty(userToken?.SubscriptionId) && string.IsNullOrEmpty(transientToken))
                    throw new TvProgException("Invalid token");
                ...
                {
                    var userInformation = new Riskv1authenticationsetupsPaymentInformationCustomer(CustomerId: userToken.SubscriptionId);
                    var paymentInformation = new Riskv1authenticationsetupsPaymentInformation(Customer: userInformation);
        ...
        public async Task<(RiskV1AuthenticationsPost201Response Result, string Error)> PayerAuthenticationEnrollmentAsync(string referenceId,
            CyberSourceUserToken userToken, string transientToken, TvProgViewer.Services.Payments.ProcessPaymentRequest processPaymentRequest, string returnUrl)
        {
            return await HandleFunctionAsync(async () =>
            {
                if (_cyberSourceSettings.PaymentConnectionMethod != ConnectionMethodType.FlexMicroForm)
                ...
                if (userToken is not null)
                {
                    var userInformation = new Ptsv2paymentsPaymentInformationCustomer(CustomerId: userToken.SubscriptionId);
                    var paymentInformation = new Riskv1authenticationsPaymentInformation(Customer: userInformation);
                    requestObj.PaymentInformation = paymentInformation;
                }
                else if (!string.IsNullOrEmpty(transientToken))
                    requestObj.TokenInformation = new Riskv1authenticationsetupsTokenInformation { TransientToken = transientToken };
        ...
        public async Task<(RiskV1AuthenticationResultsPost201Response Result, string Error)> PayerAuthenticationValidateAsync(string authenticationTransactionId)
        {
            return await HandleFunctionAsync(async () =>
            {
                ...
                if (string.IsNullOrEmpty(authenticationTransactionId))
                    throw new TvProgException("Invalid authentication transaction id");
