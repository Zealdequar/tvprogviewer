...
namespace TvProgViewer.Plugin.Widgets.FacebookPixel.Services
{
    ...
    public class FacebookPixelService
    {
        ...
        private async Task<bool> PluginActiveAsync()
        {
            ...
            return await _widgetPluginManager.IsPluginActiveAsync(FacebookPixelDefaults.SystemName, user, store.Id);
        }
        ...
        private async Task<string> GetUserObjectAsync()
        {
            ...
            return FormatEventObject(new List<(string Name, object Value)>
            {
                ...
                ("external_id", user.UserGuid.ToString().ToLowerInvariant()),
        ...
        private async Task<string> PrepareUserPropertiesScriptAsync(IList<FacebookPixelConfiguration> configurations)
        {
            ...
            //prepare user object
            var user = await _workContext.GetCurrentUserAsync();
            var createdOn = new DateTimeOffset(user.CreatedOnUtc).ToUnixTimeSeconds().ToString();
            var city = user.City;
            var countryId = user.CountryId;
            var countryName = (await _countryService.GetCountryByIdAsync(countryId))?.TwoLetterIsoCode;
        ...
        private async Task<string> PrepareTrackedEventsScriptAsync(IList<FacebookPixelConfiguration> configurations)
        {
            ...
            var activeEvents = events.Where(trackedEvent =>
                trackedEvent.UserId == user.Id && trackedEvent.StoreId == store.Id)
                .ToList();
        ...
        private async Task PrepareTrackedEventScriptAsync(string eventName, string eventObject,
            int? userId = null, int? storeId = null, bool isCustomEvent = false)
        {
            ...
            var activeEvent = events.FirstOrDefault(trackedEvent =>
                trackedEvent.EventName == eventName && trackedEvent.UserId == userId && trackedEvent.StoreId == storeId);
            if (activeEvent == null)
            {
        ...
        /// </returns>
        private async Task<ConversionsEventUserData> PrepareUserDataAsync(User user = null)
        {
            //prepare user object
            user ??= await _workContext.GetCurrentUserAsync();
            var twoLetterCountryIsoCode = (await _countryService.GetCountryByIdAsync(user.CountryId))?.TwoLetterIsoCode;
            var stateName = (await _stateProvinceService.GetStateProvinceByIdAsync(user.StateProvinceId))?.Abbreviation;
            ...
            {
                EmailAddress = new List<string> { HashHelper.CreateHash(Encoding.UTF8.GetBytes(user.Email?.ToLowerInvariant() ?? string.Empty), "SHA256") },
                FirstName = new List<string> { HashHelper.CreateHash(Encoding.UTF8.GetBytes(user.FirstName?.ToLowerInvariant() ?? string.Empty), "SHA256") },
                LastName = new List<string> { HashHelper.CreateHash(Encoding.UTF8.GetBytes(user.LastName?.ToLowerInvariant() ?? string.Empty), "SHA256") },
                PhoneNumber = new List<string> { HashHelper.CreateHash(Encoding.UTF8.GetBytes(user.SmartPhone?.ToLowerInvariant() ?? string.Empty), "SHA256") },
                ExternalId = new List<string> { HashHelper.CreateHash(Encoding.UTF8.GetBytes(user?.UserGuid.ToString()?.ToLowerInvariant() ?? string.Empty), "SHA256") },
                Gender = new List<string> { HashHelper.CreateHash(Encoding.UTF8.GetBytes(user.Gender?.FirstOrDefault().ToString() ?? string.Empty), "SHA256") },
                BirthDate = new List<string> { HashHelper.CreateHash(Encoding.UTF8.GetBytes(user.BirthDate?.ToString("yyyyMMdd") ?? string.Empty), "SHA256") },
                City = new List<string> { HashHelper.CreateHash(Encoding.UTF8.GetBytes(user.City?.ToLowerInvariant() ?? string.Empty), "SHA256") },
                State = new List<string> { HashHelper.CreateHash(Encoding.UTF8.GetBytes(stateName?.ToLowerInvariant() ?? string.Empty), "SHA256") },
                Zip = new List<string> { HashHelper.CreateHash(Encoding.UTF8.GetBytes(user.ZipPostalCode?.ToLowerInvariant() ?? string.Empty), "SHA256") },
        ...
        private async Task<ConversionsEvent> PrepareAddToCartEventModelAsync(ShoppingCartItem item)
        {
            ...
            //check whether the shopping was initiated by the user
            var user = await _workContext.GetCurrentUserAsync();

            var store = await _storeContext.GetCurrentStoreAsync();

            if (item.UserId != user.Id)
                throw new TvProgException("Shopping was not initiated by user");
            ...
            return new ConversionsEvent
            {
                Data = new List<ConversionsEventDatum>
                {
                    new ConversionsEventDatum
                    {
                        ...
                        UserData = await PrepareUserDataAsync(user),
        ...
        private async Task<ConversionsEvent> PreparePurchaseModelAsync(Order order)
        {
            ...
            //check whether the purchase was initiated by the user
            var user = await _workContext.GetCurrentUserAsync();
            if (order.UserId != user.Id)
                throw new TvProgException("Purchase was not initiated by user");
        ...
        public async Task PreparePixelScriptAsync(ConversionsEvent conversionsEvent)
        {
            await HandleFunctionAsync(async() =>
            {
                ...
                foreach (var conversionsEventData in conversionsEvent.Data)
                {
                    ...
                    var activeEvent = events.FirstOrDefault(trackedEvent =>
                        trackedEvent.EventName == conversionsEventData.EventName && trackedEvent.UserId == conversionsEventData.UserData?.Id && trackedEvent.StoreId == conversionsEventData.StoreId);
                    if (activeEvent == null)
                    {
                        activeEvent = new TrackedEvent
                        {
                            EventName = conversionsEventData.EventName,
                            UserId = conversionsEventData.UserData?.Id ?? 0,
        ...
        public async Task<string> PrepareScriptAsync()
        {
            return await HandleFunctionAsync(async () =>
            {
                ...
                var configurations = await (await GetConfigurationsAsync(store.Id)).WhereAwait(async configuration =>
                {
                    ...
                    var cookieConsentAccepted = await _genericAttributeService.GetAttributeAsync<bool>(await _workContext.GetCurrentUserAsync(),
                            TvProgUserDefaults.EuCookieLawAcceptedAttribute, store.Id);
