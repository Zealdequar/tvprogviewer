...
using TvProgViewer.Plugin.Misc.Sendinblue.Domain;
...
namespace TvProgViewer.Plugin.Misc.Sendinblue.Services
{
    ...
    public class SendinblueManager
    {
        ...
        private async Task<IList<(NotifyType Type, string Message)>> ImportContactsAsync(IList<int> storeIds)
        {
            ...
            try
            {
                ...
                foreach (var storeId in storeIds)
                {
                    ...
                    var title =
                        ...
                        $"{SendinblueDefaults.SmartPhoneServiceAttribute};" +
                        ...
                        $"{SendinblueDefaults.BirthDateServiceAttribute};" +
                    ...
                    var csv = await subscriptions.AggregateAwaitAsync(title, async (all, subscription) =>
                    {
                        ...
                        if (user != null)
                        {
                            ...
                            city = user.City;
                            county = user.County;
                            state = (await _stateProvinceService.GetStateProvinceByIdAsync(user.StateProvinceId))?.Name;
                            fax = user.Fax;
                            language = await _languageService.GetLanguageByIdAsync(user.LanguageId ?? 0);
                        }
            ...
            {
                //log full error
                await _logger.ErrorAsync($"Sendinblue synchronization error: {exception.Message}", exception, await _workContext.GetCurrentUserAsync());
        ...
        private async Task<string> CreateAttibutesAsync(IList<(CategoryEnum Category, string Name, string Value, CreateAttribute.TypeEnum? Type)> attributes)
        {
            ...
            {
                //log full error
                await _logger.ErrorAsync($"Sendinblue error: {exception.Message}.", exception, await _workContext.GetCurrentUserAsync());
        ...
        public async System.Threading.Tasks.Task SubscribeAsync(NewsLetterSubscription subscription)
        {
            try
            {
                ...
                {
                    [SendinblueDefaults.UsernameServiceAttribute] = user?.Username,
                    [SendinblueDefaults.SMSServiceAttribute] = sms,
                    [SendinblueDefaults.SmartPhoneServiceAttribute] = phone,
                    ...
                    [SendinblueDefaults.BirthDateServiceAttribute] = dateOfBirth,
        ...
        public async System.Threading.Tasks.Task UpdateContactAfterCompletingOrderAsync(Order order)
        {
            try
            {
                ...
                await client.UpdateContactAsync(user.Email, updateContact);
            }
            catch (Exception exception)
            {
                //log full error
        ...
        public async Task<SendinblueAccountLanguage> GetAccountLanguageAsync()
        {
            try
            {
                ...
                //Create default user names attribytes
        ...
        public async Task<string> PrepareAttributesAsync()
        {
            try
            {
                ...
                var initialAttributes = new List<(CategoryEnum category, string Name, string Value, CreateAttribute.TypeEnum? Type)>
                {
                    ...
                    (CategoryEnum.Normal, SendinblueDefaults.SmartPhoneServiceAttribute, null, CreateAttribute.TypeEnum.Text),
                    ...
                    (CategoryEnum.Normal, SendinblueDefaults.BirthDateServiceAttribute, null, CreateAttribute.TypeEnum.Text),
        ...
        public async Task<int?> GetTemplateIdAsync(int? templateId, MessageTemplate message, EmailAccount emailAccount)
        {
            try
            {
                ...
                //or create new one
                if (emailAccount == null)
                    throw new TvProgException("Email account not configured");
        ...
        public async Task<QueuedEmail> GetQueuedEmailFromTemplateAsync(int templateId)
        {
            try
            {
                ...
                if (templateId == 0)
                    throw new TvProgException("Message template is empty");
            ...
            {
                //log full error
                await _logger.ErrorAsync($"Sendinblue email sending error: {exception.Message}.", exception, await _workContext.GetCurrentUserAsync());
                return null;
            }
        }
        ...
        /// Send SMS 
        /// </summary>
        /// <param name="to">SmartPhone number of the receiver</param>
        ...
        public async System.Threading.Tasks.Task SendSMSAsync(string to, string from, string text)
        {
            ...
            try
            {
                ...
                if (string.IsNullOrEmpty(to) || string.IsNullOrEmpty(text))
                    throw new TvProgException("SmartPhone number or SMS text is empty");
            ...
            {
                //log full error
                await _logger.ErrorAsync($"Sendinblue SMS sending error: {exception.Message}.", exception, await _workContext.GetCurrentUserAsync());
            }
        }
        ...
        public async Task<string> SendSMSCampaignAsync(int listId, string from, string text)
        {
            try
            {
                ...
                if (listId == 0 || string.IsNullOrEmpty(text) || string.IsNullOrEmpty(from))
                    throw new TvProgException("List or SMS text or sender name is empty");
