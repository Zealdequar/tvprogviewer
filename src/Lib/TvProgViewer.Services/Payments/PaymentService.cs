...
namespace TvProgViewer.Services.Payments
{
    ...
    public partial class PaymentService : IPaymentService
    {
        ...
        public virtual async Task PostProcessPaymentAsync(PostProcessPaymentRequest postProcessPaymentRequest)
        {
            ...
            var user = await _userService.GetUserByIdAsync(postProcessPaymentRequest.Order.UserId);
            var paymentMethod = await _paymentPluginManager
                .LoadPluginBySystemNameAsync(postProcessPaymentRequest.Order.PaymentMethodSystemName, user, postProcessPaymentRequest.Order.StoreId)
                ?? throw new TvProgException("Payment method couldn't be loaded");
        ...
        public virtual async Task<bool> CanRePostProcessPaymentAsync(Order order)
        {
            ...
            var paymentMethod = await _paymentPluginManager.LoadPluginBySystemNameAsync(order.PaymentMethodSystemName, user, order.StoreId);
        ...
        public virtual async Task<decimal> GetAdditionalHandlingFeeAsync(IList<ShoppingCartItem> cart, string paymentMethodSystemName)
        {
            ...
            var paymentMethod = await _paymentPluginManager.LoadPluginBySystemNameAsync(paymentMethodSystemName, user, cart.FirstOrDefault()?.StoreId ?? 0);
            if (paymentMethod == null)
                return decimal.Zero;
