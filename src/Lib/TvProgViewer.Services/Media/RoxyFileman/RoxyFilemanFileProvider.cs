...
namespace TvProgViewer.Services.Media.RoxyFileman
{
    /// <summary>
    ...
    public class RoxyFilemanFileProvider : PhysicalFileProvider, IRoxyFilemanFileProvider
    {
        ...
        protected ITvProgFileProvider _tvProgFileProvider;
        ...
        public RoxyFilemanFileProvider(ITvProgFileProvider tvProgFileProvider) : base(tvProgFileProvider.Combine(tvProgFileProvider.WebRootPath, TvProgRoxyFilemanDefaults.DefaultRootDirectory))
        {
            _tvProgFileProvider = tvProgFileProvider;
        }

        public RoxyFilemanFileProvider(ITvProgFileProvider defaultFileProvider, MediaSettings mediaSettings) : this(defaultFileProvider)
        {
        ...
        public virtual async Task<RoxyFilemanConfig> GetOrCreateConfigurationAsync(string pathBase, string lang)
        {
            ...
            var filePath = _tvProgFileProvider.GetAbsolutePath(TvProgRoxyFilemanDefaults.ConfigurationFile);

            //create file if not exists
            _tvProgFileProvider.CreateFile(filePath);

            //try to read existing configuration
            var existingText = await _tvProgFileProvider.ReadAllTextAsync(filePath, Encoding.UTF8);
            ...
            {
                FILES_ROOT = existingConfiguration?.FILES_ROOT ?? TvProgRoxyFilemanDefaults.DefaultRootDirectory,
                ...
                FORBIDDEN_UPLOADS = existingConfiguration?.FORBIDDEN_UPLOADS ?? string.Join(" ", TvProgRoxyFilemanDefaults.ForbiddenUploadExtensions),
