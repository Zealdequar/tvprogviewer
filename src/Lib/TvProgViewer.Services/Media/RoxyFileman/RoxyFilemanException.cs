...
namespace TvProgViewer.Services.Media.RoxyFileman
{
    ...
    public class RoxyFilemanException : Exception
    {
        ...
        protected static string GetLocalizedMessage(string key)
        {
            ...
            var languageFile = fileProvider.GetAbsolutePath($"{TvProgRoxyFilemanDefaults.LanguageDirectory}/{roxyConfig.LANG}.json");

            if (!fileProvider.FileExists(languageFile))
                languageFile = fileProvider.GetAbsolutePath($"{TvProgRoxyFilemanDefaults.LanguageDirectory}/en.json");
