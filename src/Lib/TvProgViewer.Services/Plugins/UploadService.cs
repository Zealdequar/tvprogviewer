...
namespace TvProgViewer.Services.Plugins
{
    ...
    public partial class UploadService : IUploadService
    {
        ...
        public UploadService(ITvProgFileProvider fileProvider,
        ...
        protected virtual async Task<IList<UploadedItem>> GetUploadedItemsAsync(string archivePath)
        {
            ...
            var uploadedItemsFileEntry = archive.Entries
                .FirstOrDefault(entry => entry.Name.Equals(TvProgPluginDefaults.UploadedItemsFileName, StringComparison.InvariantCultureIgnoreCase)
        ...
        protected virtual async Task<IDescriptor> UploadSingleItemAsync(string archivePath)
        {
            ...
            var pluginsDirectory = _fileProvider.MapPath(TvProgPluginDefaults.UploadedPath);
            ...
            if (!string.IsNullOrEmpty(TvProgPluginDefaults.ThemesPath))
                themesDirectory = _fileProvider.MapPath(TvProgPluginDefaults.ThemesPath);
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                ...
                if (rootDirectories.Count != 1)
                {
                    throw new Exception("The archive should contain only one root plugin or theme directory. " +
                        ...
                        $"To upload multiple items, the archive should have the '{TvProgPluginDefaults.UploadedItemsFileName}' file in the root");
                }
                ...
                foreach (var entry in archive.Entries)
                {
                    ...
                    var isPluginDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{TvProgPluginDefaults.DescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);

                    //or whether it's a theme descriptor
                    var isThemeDescriptor = entry.FullName
                        .Equals($"{uploadedItemDirectoryName}/{TvProgPluginDefaults.ThemeDescriptionFileName}", StringComparison.InvariantCultureIgnoreCase);
        ...
        protected virtual async Task<IList<IDescriptor>> UploadMultipleItemsAsync(string archivePath, IList<UploadedItem> uploadedItems)
        {
            ...
            using (var archive = ZipFile.OpenRead(archivePath))
            {
                foreach (var item in uploadedItems)
                {
                    ...
                    //ensure that the current version of tvProgViewer is supported
                    ...
                    if (item.Type == UploadedItemType.Plugin)
                        descriptorPath = $"{itemPath}{TvProgPluginDefaults.DescriptionFileName}";

                    if (item.Type == UploadedItemType.Theme && !string.IsNullOrEmpty(TvProgPluginDefaults.ThemeDescriptionFileName))
                        descriptorPath = $"{itemPath}{TvProgPluginDefaults.ThemeDescriptionFileName}";
        ...
        public virtual async Task<IList<IDescriptor>> UploadPluginsAndThemesAsync(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var tempDirectory = _fileProvider.MapPath(TvProgPluginDefaults.UploadsTempPath);
        ...
        public virtual async Task UploadIconsArchiveAsync(IFormFile archivefile)
        {
            ...
            try
            {
                ...
                var storeIconsPath = _fileProvider.GetAbsolutePath(string.Format(TvProgCommonDefaults.FaviconAndAppIconsPath, await _storeContext.GetActiveStoreScopeConfigurationAsync()));
        ...
        public virtual async Task UploadFaviconAsync(IFormFile favicon)
        {
            ...
            var storeFaviconPath = _fileProvider.GetAbsolutePath(string.Format(TvProgCommonDefaults.FaviconAndAppIconsPath, await _storeContext.GetActiveStoreScopeConfigurationAsync()));
        ...
        public virtual Task UploadLocalePatternAsync(CultureInfo cultureInfo = null)
        {
            ...
            try
            {
                ...
                var ziplocalePatternPath = getPath(TvProgCommonDefaults.LocalePatternPath, TvProgCommonDefaults.LocalePatternArchiveName);
                    ...
                    checkDirectoryExists(TvProgCommonDefaults.LocalePatternPath, currentCulture.TwoLetterISOLanguageName)))
                {
                    ...
                    ZipFile.ExtractToDirectory(ziplocalePatternPath, getPath(TvProgCommonDefaults.LocalePatternPath, tempFolder));

                    //4. Search in the temp unpacked archive a folder with locales by culture
                    var sourcelocalePath = _fileProvider.Combine(getPath(TvProgCommonDefaults.LocalePatternPath, tempFolder), currentCulture.Name);
                    if (_fileProvider.DirectoryExists(sourcelocalePath))
                    {
                        cultureToUse = currentCulture.Name;
                    }

                    var sourcelocaleISOPath = _fileProvider.Combine(getPath(TvProgCommonDefaults.LocalePatternPath, tempFolder), currentCulture.TwoLetterISOLanguageName);
                    ...
                    {
                        var destlocalePath = getPath(TvProgCommonDefaults.LocalePatternPath, cultureToUse);
            ...
            finally
            {
                ...
                if (checkDirectoryExists(TvProgCommonDefaults.LocalePatternPath, tempFolder))
                    _fileProvider.DeleteDirectory(getPath(TvProgCommonDefaults.LocalePatternPath, tempFolder));
            }
        ...
        public partial class UploadedItem
        {
            ...
            /// <summary>
            /// Gets or sets supported versions of tvProgViewer
            /// </summary>
