...
namespace TvProgViewer.Services.Localization
{
    ...
    public partial class LocalizationService : ILocalizationService
    {
        ...
        protected virtual async Task<IDictionary<string, string>> UpdateLocaleResourceAsync(IDictionary<string, string> resources, int? languageId = null, bool clearCache = true)
        {
            ...
            //clear cache
            if (clearCache)
                await _staticCacheManager.RemoveByPrefixAsync(TvProgEntityCacheDefaults<LocaleStringResource>.Prefix);
        ...
        protected virtual IDictionary<string, string> UpdateLocaleResource(IDictionary<string, string> resources, int? languageId = null, bool clearCache = true)
        {
            ...
            //clear cache
            if (clearCache)
                _staticCacheManager.RemoveByPrefix(TvProgEntityCacheDefaults<LocaleStringResource>.Prefix);
        ...
        public virtual async Task<Dictionary<string, KeyValuePair<int, string>>> GetAllResourceValuesAsync(int languageId, bool? loadPublicLocales)
        {
            ...
            return await _staticCacheManager.GetAsync(key, () =>
            {
                ...
                var query = from l in _lsrRepository.Table
                            ...
                            where l.LanguageId == languageId
                            select l;
                query = loadPublicLocales.Value ? query.Where(r => !r.ResourceName.StartsWith(TvProgLocalizationDefaults.AdminLocaleStringResourcesPrefix)) : query.Where(r => r.ResourceName.StartsWith(TvProgLocalizationDefaults.AdminLocaleStringResourcesPrefix));
            ...
            bool logIfNotFound = true, string defaultValue = "", bool returnEmptyIfNotFound = false)
        {
            ...
            if (_localizationSettings.LoadAllLocaleRecordsOnStartup)
            {
                ...
                var resources = await GetAllResourceValuesAsync(languageId, !resourceKey.StartsWith(TvProgLocalizationDefaults.AdminLocaleStringResourcesPrefix, StringComparison.InvariantCultureIgnoreCase));
            ...
            {
                //gradual loading
                var key = _staticCacheManager.PrepareKeyForDefaultCache(TvProgLocalizationDefaults.LocaleStringResourcesByNameCacheKey
        ...
        public virtual async Task<string> ExportResourcesToXmlAsync(Language language)
        {
            ...
            await xmlWriter.WriteAttributeStringAsync("SupportedVersion", TvProgVersion.CURRENT_VERSION);
        ...
        public virtual async Task<string> GetLocalizedEnumAsync<TEnum>(TEnum enumValue, int? languageId = null) where TEnum : struct
        {
            ...
            //localized value
            var workingLanguage = await _workContext.GetWorkingLanguageAsync();
            var resourceName = $"{TvProgLocalizationDefaults.EnumLocaleStringResourcesPrefix}{typeof(TEnum)}.{enumValue}";
        ...
        public virtual async Task<string> GetLocalizedPermissionNameAsync(PermissionRecord permissionRecord, int? languageId = null)
        {
            ...
            //localized value
            var workingLanguage = await _workContext.GetWorkingLanguageAsync();
            var resourceName = $"{TvProgLocalizationDefaults.PermissionLocaleStringResourcesPrefix}{permissionRecord.SystemName}";
            var result = await GetResourceAsync(resourceName, languageId ?? workingLanguage.Id, false, string.Empty, true);
            ...
            where TPlugin : IPlugin
        {
            ...
            //localized value
            var resourceName = $"{TvProgLocalizationDefaults.PluginNameLocaleStringResourcesPrefix}{systemName}";
