    ...
    public partial class DiscountService : IDiscountService
    {
            ...
            RequirementGroupInteractionType groupInteractionType, User user, List<string> errors)
        {
            ...
            foreach (var requirement in requirements)
            {
                if (requirement.IsGroup)
                {
                    ...
                    result = await GetValidationResultAsync(childRequirements, interactionType, user, errors);
                }
                else
                {
                    ...
                    var requirementRulePlugin = await _discountPluginManager
                        .LoadPluginBySystemNameAsync(requirement.DiscountRequirementRuleSystemName, user, store.Id);
            ...
            DateTime? startDateUtc = null, DateTime? endDateUtc = null, bool? isActive = true)
        {
            ...
            var discounts = (await _discountRepository.GetAllAsync(query =>
            {
                ...
                return query;
            }, cache => cache.PrepareKeyForDefaultCache(TvProgDiscountDefaults.DiscountAllCacheKey, 
        ...
        public virtual async Task<IList<Discount>> GetAppliedDiscountsAsync<T>(IDiscountSupported<T> entity) where T : DiscountMapping
        {
            ...
            var cacheKey = _staticCacheManager.PrepareKeyForShortTermCache(TvProgDiscountDefaults.AppliedDiscountsCacheKey, entity.GetType().Name, entity);
        ...
        public virtual async Task<DiscountValidationResult> ValidateDiscountAsync(Discount discount, User user)
        {
            ...
            var couponCodesToValidate = await _userService.ParseAppliedDiscountCouponCodesAsync(user);
            
            return await ValidateDiscountAsync(discount, user, couponCodesToValidate);
        }
        ...
        public virtual async Task<DiscountValidationResult> ValidateDiscountAsync(Discount discount, User user, string[] couponCodesToValidate)
        {
                ...
                case DiscountLimitationType.NTimesPerUser:
                    {
                        ...
                        {
                            var usedTimes = (await GetAllDiscountUsageHistoryAsync(discount.Id, user.Id, null, false, 0, 1)).TotalCount;
                            if (usedTimes >= discount.LimitationTimes)
                            {
            ...
            var key = _staticCacheManager.PrepareKeyForDefaultCache(TvProgDiscountDefaults.DiscountRequirementsByDiscountCacheKey, discount);
            ...
            result.IsValid = await GetValidationResultAsync(requirements, topLevelGroup.InteractionType.Value, user, errors);
        ...
        public virtual async Task<IPagedList<DiscountUsageHistory>> GetAllDiscountUsageHistoryAsync(int? discountId = null,
            int? userId = null, int? orderId = null, bool includeCancelledOrders = true, int pageIndex = 0, int pageSize = int.MaxValue)
        {
