...
namespace TvProgViewer.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        public virtual async Task<IList<Product>> GetCategoryFeaturedProductsAsync(int categoryId, int storeId = 0)
        {
            ...
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(TvProgCatalogDefaults.CategoryFeaturedProductsIdsKey, categoryId, userRoleIds, storeId);
        ...
        public virtual async Task<IList<Product>> GetManufacturerFeaturedProductsAsync(int manufacturerId, int storeId = 0)
        {
            ...
            var user = await _workContext.GetCurrentUserAsync();
            var userRoleIds = await _userService.GetUserRoleIdsAsync(user);
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(TvProgCatalogDefaults.ManufacturerFeaturedProductIdsKey, manufacturerId, userRoleIds, storeId);
        ...
        public virtual async Task<int> GetNumberOfProductsInCategoryAsync(IList<int> categoryIds = null, int storeId = 0)
        {
            ...
            var cacheKey = _staticCacheManager
                .PrepareKeyForDefaultCache(TvProgCatalogDefaults.CategoryProductsNumberCacheKey, userRoleIds, storeId, categoryIds);
            ...
            bool? overridePublished = null)
        {
            ...
            if (!showHidden)
            {
                ...
                //apply ACL constraints
                var user = await _workContext.GetCurrentUserAsync();
                productsQuery = await _aclService.ApplyAcl(productsQuery, user);
            }
        ...
        /// <returns>A task that represents the asynchronous operation</returns>
        public virtual async Task<IList<TierPrice>> GetTierPricesAsync(Product product, User user, Store store)
        {
            if (product is null)
                throw new ArgumentNullException(nameof(product));
        ...
        /// </returns>
        public virtual async Task<TierPrice> GetPreferredTierPriceAsync(Product product, User user, Store store, int quantity)
        {
            if (product is null)
                throw new ArgumentNullException(nameof(product));
            ...
            return (await GetTierPricesAsync(product, user, store))?.LastOrDefault(price => quantity >= price.Quantity);
        }
        ...
        public virtual async Task SetProductReviewHelpfulnessAsync(ProductReview productReview, bool helpfulness)
        {
            ...
            var prh = _productReviewHelpfulnessRepository.Table
                .SingleOrDefault(h => h.ProductReviewId == productReview.Id && h.UserId == user.Id);
        ...
        public virtual async Task<bool> CanAddReviewAsync(int productId, int storeId = 0)
        {
            ...
            if (_catalogSettings.OneReviewPerProductFromUser)
                return (await GetAllProductReviewsAsync(userId: user.Id, productId: productId, storeId: storeId)).TotalCount == 0;
