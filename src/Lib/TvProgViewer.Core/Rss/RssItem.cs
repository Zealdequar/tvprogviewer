...
namespace TvProgViewer.Core.Rss
{
    ...
    public partial class RssItem
    {
        ...
        public RssItem(string title, string content, Uri link, string id, DateTimeOffset pubDate)
        {
            ...
            Content = new XElement(TvProgRssDefaults.Description, content);
            Link = new XElement(TvProgRssDefaults.Link, link);
            Id = new XElement(TvProgRssDefaults.Guid, new XAttribute("isPermaLink", false), id);
            PubDate = new XElement(TvProgRssDefaults.PubDate, pubDate.ToString("r"));
        }
        ...
        {
            var title = item.Element(TvProgRssDefaults.Title)?.Value ?? string.Empty;
            var content = item.Element(TvProgRssDefaults.Content)?.Value ?? string.Empty;
            if (string.IsNullOrEmpty(content))
                content = item.Element(TvProgRssDefaults.Description)?.Value ?? string.Empty;
            var link = new Uri(item.Element(TvProgRssDefaults.Link)?.Value ?? string.Empty);
            var pubDateValue = item.Element(TvProgRssDefaults.PubDate)?.Value;
            var pubDate = pubDateValue == null ? DateTimeOffset.Now : DateTimeOffset.ParseExact(pubDateValue, "r", null);
            var id = item.Element(TvProgRssDefaults.Guid)?.Value ?? string.Empty;
        ...
        {
            var element = new XElement(TvProgRssDefaults.Item, Id, Link, Title, Content);
