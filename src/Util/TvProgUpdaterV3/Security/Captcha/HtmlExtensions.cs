...
namespace TvProgViewer.TvProgUpdaterV3.Security.Captcha
{
    /// <summary>
    /// HTML extensions
    /// </summary>
    public static class HtmlExtensions
    {
        ...
        private static TagBuilder GenerateLoadApiScriptTag(CaptchaSettings captchaSettings, string captchaId, string render, string language)
        {
            var hl = !string.IsNullOrEmpty(language)
                ? $"&hl={language}"
                : string.Empty;
            var url = string.Format($"{captchaSettings.ReCaptchaApiUrl}{TvProgSecurityDefaults.RecaptchaScriptPath}", captchaId, render, hl);
