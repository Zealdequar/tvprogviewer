    ...
    public sealed class SignOutFromExternalAuthenticationAttribute : TypeFilterAttribute
    {
        ...
        private class SignOutFromExternalAuthenticationFilter : IAuthorizationFilter
        {
            ...
            public async void OnAuthorization(AuthorizationFilterContext filterContext)
            {
                ...
                var authenticateResult = await filterContext.HttpContext.AuthenticateAsync(TvProgAuthenticationDefaults.ExternalAuthenticationScheme);
                if (authenticateResult.Succeeded)
                    await filterContext.HttpContext.SignOutAsync(TvProgAuthenticationDefaults.ExternalAuthenticationScheme);
            }
