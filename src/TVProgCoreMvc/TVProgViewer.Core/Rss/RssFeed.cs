...
namespace TVProgViewer.Core.Rss
{
    /// <summary>
    ...
    public partial class RssFeed
    {
        ...
        {
            Title = new XElement(TvProgRssDefaults.Title, title);
            Description = new XElement(TvProgRssDefaults.Description, description);
            Link = new XElement(TvProgRssDefaults.Link, link);
            LastBuildDate = new XElement(TvProgRssDefaults.LastBuildDate, lastBuildDate.ToString("r"));
        }
        ...
        public static async Task<RssFeed> LoadAsync(Stream stream)
        {
            try
            {
                ...
                var channel = document.Root?.Element(TvProgRssDefaults.Channel);

                if (channel == null)
                    return null;

                var title = channel.Element(TvProgRssDefaults.Title)?.Value ?? string.Empty;
                var description = channel.Element(TvProgRssDefaults.Description)?.Value ?? string.Empty;
                var link = new Uri(channel.Element(TvProgRssDefaults.Link)?.Value ?? string.Empty);
                var lastBuildDateValue = channel.Element(TvProgRssDefaults.LastBuildDate)?.Value;
                ...
                foreach (var item in channel.Elements(TvProgRssDefaults.Item))
                {
        ...
        public string GetContent()
        {
            ...
            var root = new XElement(TvProgRssDefaults.RSS, new XAttribute("version", "2.0"));
            var channel = new XElement(TvProgRssDefaults.Channel,
