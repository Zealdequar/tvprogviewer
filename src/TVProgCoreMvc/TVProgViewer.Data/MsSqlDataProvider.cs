...
namespace TVProgViewer.Data
{
    ...
    public partial class MsSqlDataProvider : BaseDataProvider, IDataProvider
    {
        ...
        protected void ExecuteSqlScriptFromFile(ITvProgFileProvider fileProvider, string filePath)
        {
        ...
        public void ExecuteSqlScript(string sql)
        {
            ...
            using (var currentConnection = new TvProgDataConnection())
            {
        ...
        public void InitializeDatabase()
        {
            ...
            ExecuteSqlScriptFromFile(fileProvider, TvProgDataDefaults.SqlServerStoredProceduresFilePath);
        }
        ...
        public virtual void BackupDatabase(string fileName)
        {
            ...
            //var fileName = _fileProvider.Combine(GetBackupDirectoryPath(), $"database_{DateTime.Now:yyyy-MM-dd-HH-mm-ss}_{CommonHelper.GenerateRandomDigitCode(10)}.{TvProgCommonDefaults.DbBackupFileExtension}");
        ...
        public virtual string BuildConnectionString(ITVProgViewerConnectionStringInfo TVProgViewerConnectionString)
        {
            if (TVProgViewerConnectionString is null)
                throw new ArgumentNullException(nameof(TVProgViewerConnectionString));

            var builder = new SqlConnectionStringBuilder
            {
                DataSource = TVProgViewerConnectionString.ServerName,
                InitialCatalog = TVProgViewerConnectionString.DatabaseName,
                PersistSecurityInfo = false,
                IntegratedSecurity = TVProgViewerConnectionString.IntegratedSecurity,
                ConnectTimeout = 99999900
            };

            if (!TVProgViewerConnectionString.IntegratedSecurity)
            {
                builder.UserID = TVProgViewerConnectionString.Username;
                builder.Password = TVProgViewerConnectionString.Password;
            }
