...
using TVProgViewer.Data.DataProviders.LinqToDB;
using TVProgViewer.Data.Migrations;
using Npgsql;

namespace TVProgViewer.Data.DataProviders
{
    public class PostgreSqlDataProvider : BaseDataProvider, ITvProgDataProvider
    {
        #region Utils
        ...
        private string GetSequenceName<TEntity>(DataConnection dataConnection) where TEntity : BaseEntity
        {
            ...
            if (descriptor is null)
                throw new TvProgException($"Mapped entity descriptor is not found: {typeof(TEntity).Name}");
            ...
            if (string.IsNullOrEmpty(columnName))
                throw new TvProgException("A table's primary key does not have an identity constraint");
        ...
        public void InitializeDatabase()
        {
            ...
            migrationManager.ApplyUpMigrations(typeof(TvProgDbStartup).Assembly);
        }
        ...
        public virtual string BuildConnectionString(ITvProgConnectionStringInfo nopConnectionString)
        {
            if (nopConnectionString is null)
                throw new ArgumentNullException(nameof(nopConnectionString));

            if (nopConnectionString.IntegratedSecurity)
                throw new TvProgException("Data provider supports connection only with login and password");
