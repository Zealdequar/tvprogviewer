...
{
    /// <summary>
    /// Represents the official feed manager (plugins from TVProgViewercommerce marketplace)
    /// </summary>
    public partial class OfficialFeedManager
    {
        ...
        private readonly TvProgHttpClient _TvProgHttpClient;
        ...
        public OfficialFeedManager(ILogger logger,
            TvProgHttpClient TvProgHttpClient)
        {
            _logger = logger;
            _TvProgHttpClient = TvProgHttpClient;
        }
        ...
        public virtual IList<OfficialFeedCategory> GetCategories()
        {
            ...
            try
            {
                xml.LoadXml(_TvProgHttpClient.GetExtensionsCategoriesAsync().Result);
            }
            catch (Exception ex)
            {
                _logger.Error("No access to the list of plugins. Website www.TVProgViewercommerce.com is not available.", ex);
            }
        ...
        public virtual IList<OfficialFeedVersion> GetVersions()
        {
            ...
            try
            {
                xml.LoadXml(_TvProgHttpClient.GetExtensionsVersionsAsync().Result);
            }
            catch (Exception ex)
            {
                _logger.Error("No access to the list of plugins. Website www.TVProgViewercommerce.com is not available.", ex);
            ...
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            try
            {
                xml.LoadXml(_TvProgHttpClient.GetExtensionsAsync(categoryId, versionId, price, searchTerm, pageIndex, pageSize).Result);
            }
            catch (Exception ex)
            {
                _logger.Error("No access to the list of plugins. Website www.TVProgViewercommerce.com is not available.", ex);
