...
namespace TVProgViewer.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
            ...
            var products = await _productRepository.GetAllAsync(query =>
            {
                return from p in query
                       ...
                       where p.Published &&
                             ...
                             p.ShowOnHomepage
                       select p;
            }, cache => cache.PrepareKeyForDefaultCache(TvProgCatalogDefaults.ProductsHomepageCacheKey));
        ...
        public virtual async Task<IList<Product>> GetCategoryFeaturedProductsAsync(int categoryId, int storeId = 0)
        {
            ...
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(TvProgCatalogDefaults.CategoryFeaturedProductsIdsKey, categoryId, userRolesIds, storeId);
        ...
        public virtual async Task<IList<Product>> GetManufacturerFeaturedProductsAsync(int manufacturerId, int storeId = 0)
        {
            ...
            var user = await _workContext.GetCurrentUserAsync();
            var userRolesIds = await _userService.GetUserRoleIdsAsync(user);
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(TvProgCatalogDefaults.ManufacturerFeaturedProductIdsKey, manufacturerId, userRolesIds, storeId);
        ...
        public virtual async Task<int> GetNumberOfProductsInCategoryAsync(IList<int> categoryIds = null, int storeId = 0)
        {
            ...
            var cacheKey = _staticCacheManager
                .PrepareKeyForDefaultCache(TvProgCatalogDefaults.CategoryProductsNumberCacheKey, userRolesIds, storeId, categoryIds);
            ...
            bool? overridePublished = null)
        {
            ...
            if (!showHidden)
            {
                var user = await _workContext.GetCurrentUserAsync();
                var userRolesIds = await _userService.GetUserRoleIdsAsync(user);
                productsQuery = await FilterHiddenEntriesAsync(productsQuery, storeId, userRolesIds);
            }
        ...
        public virtual async Task<IList<RelatedProduct>> GetRelatedProductsByProductId1Async(int productId, bool showHidden = false)
        {
            ...
            var relatedProducts = await _staticCacheManager.GetAsync(_staticCacheManager.PrepareKeyForDefaultCache(TvProgCatalogDefaults.RelatedProductsCacheKey, productId, showHidden), async () => await query.ToListAsync());
        ...
        /// <summary>
        /// Gets a product tier prices for user
        /// </summary>
        /// <param name="product">Product</param>
        /// <param name="user">User</param>
        /// <param name="storeId">Store identifier</param>
        public virtual async Task<IList<TierPrice>> GetTierPricesAsync(Product product, User user, int storeId)
        {
            if (product is null)
                throw new ArgumentNullException(nameof(product));
        ...
        public virtual async Task<IList<TierPrice>> GetTierPricesByProductAsync(int productId)
        {
            ...
            return await _staticCacheManager.GetAsync(_staticCacheManager.PrepareKeyForDefaultCache(TvProgCatalogDefaults.TierPricesByProductCacheKey, productId), async () => await query.ToListAsync());
        }
        ...
        /// <returns>Tier price</returns>
        public virtual async Task<TierPrice> GetPreferredTierPriceAsync(Product product, User user, int storeId, int quantity)
        {
            if (product is null)
                throw new ArgumentNullException(nameof(product));
            ...
            return (await GetTierPricesAsync(product, user, storeId))?.LastOrDefault(price => quantity >= price.Quantity);
        }
        ...
        public virtual async Task<IPagedList<ProductReview>> GetAllProductReviewsAsync(int userId = 0, bool? approved = null,
        ...
        public virtual async Task SetProductReviewHelpfulnessAsync(ProductReview productReview, bool helpfulness)
        {
            ...
            var prh = await _productReviewHelpfulnessRepository.Table
                .SingleOrDefaultAwaitAsync(async h => h.ProductReviewId == productReview.Id && h.UserId == (await _workContext.GetCurrentUserAsync()).Id);
        ...
        {
            if (_catalogSettings.OneReviewPerProductFromUser)
                return (await GetAllProductReviewsAsync(userId: (await _workContext.GetCurrentUserAsync()).Id, productId: productId, storeId: storeId)).TotalCount == 0;
