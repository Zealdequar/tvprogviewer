...
namespace TVProgViewer.Services.Catalog
{
    ...
    public partial class ProductService : IProductService
    {
        ...
        protected readonly IUserService _customerService;
        ...
        public virtual IList<Product> GetAllProductsDisplayedOnHomepage()
        {
            ...
            var products = query.ToCachedList(TvProgCatalogCachingDefaults.ProductsAllDisplayedOnHomepageCacheKey);
        ...
        public virtual IList<Product> GetProductsByIds(int[] productIds)
        {
            ...
            var key = TvProgCatalogCachingDefaults.ProductsByIdsCacheKey.FillCacheKey(productIds);
        ...
        public virtual int GetNumberOfProductsInCategory(IList<int> categoryIds = null, int storeId = 0)
        {
            ...
            var allowedUserRolesIds = new List<int>();

            if (!_catalogSettings.IgnoreAcl)
            {
                //Access control list. Allowed customer roles
                allowedUserRolesIds.AddRange(_customerService.GetUserRoleIds(_workContext.CurrentUser));
                ...
                query = from p in query
                        ...
                        where !p.SubjectToAcl || allowedUserRolesIds.ToArray().Contains(acl.UserRoleId)
                        select p;
            }
            else
            {
                allowedUserRolesIds.Add(0);
            }
            ...
            var cacheKey = TvProgCatalogCachingDefaults.CategoryNumberOfProductsCacheKey
                .FillCacheKey(allowedUserRolesIds, storeId, categoryIds);
            ...
            bool? overridePublished = null)
        {
            ...
            //Access control list. Allowed customer roles
            var allowedUserRolesIds = _customerService.GetUserRoleIds(_workContext.CurrentUser);

            //pass category identifiers as comma-delimited string
            var commaSeparatedCategoryIds = categoryIds == null ? string.Empty : string.Join(",", categoryIds);

            //pass customer role identifiers as comma-delimited string
            var commaSeparatedAllowedUserRoleIds = string.Join(",", allowedUserRolesIds);
            ...
            var pAllowedUserRoleIds = SqlParameterHelper.GetStringParameter("AllowedUserRoleIds", !_catalogSettings.IgnoreAcl ? commaSeparatedAllowedUserRoleIds : string.Empty);
            ...
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            var key = TvProgCatalogCachingDefaults.ProductsByProductAtributeCacheKey.FillCacheKey(productAttributeId);
        ...
        public virtual IList<RelatedProduct> GetRelatedProductsByProductId1(int productId, bool showHidden = false)
        {
            ...
            var relatedProducts = query.ToCachedList(TvProgCatalogCachingDefaults.ProductsRelatedCacheKey.FillCacheKey(productId, showHidden));
        ...
        public virtual IList<TierPrice> GetTierPrices(Product product, User customer, int storeId)
        {
            if (product is null)
                throw new ArgumentNullException(nameof(product));
            ...
            if (!_catalogSettings.IgnoreAcl)
            {
                var customerRoleIds = _customerService.GetUserRoleIds(customer);

                actualTierPrices = actualTierPrices.Where(tierPrice =>
                   !tierPrice.UserRoleId.HasValue ||
                   tierPrice.UserRoleId.Value == 0 ||
                   customerRoleIds.Contains(tierPrice.UserRoleId.Value));
            }
        ...
        public virtual IList<TierPrice> GetTierPricesByProduct(int productId)
        {
            return _tierPriceRepository.Table.Where(tp => tp.ProductId == productId)
                .ToCachedList(TvProgCatalogCachingDefaults.ProductTierPricesCacheKey.FillCacheKey(productId));
        }
        ...
        /// <param name="quantity">Quantity</param>
        /// <returns>Tier price</returns>
        public virtual TierPrice GetPreferredTierPrice(Product product, User customer, int storeId, int quantity)
        {
            if (product is null)
                throw new ArgumentNullException(nameof(product));
        ...
        /// </summary>
        /// <param name="customerId">User identifier (who wrote a review); 0 to load all records</param>
            ...
            int pageIndex = 0, int pageSize = int.MaxValue)
        {
            ...
            if (customerId > 0)
                query = query.Where(pr => pr.UserId == customerId);
            if (fromUtc.HasValue)
        ...
        public virtual void SetProductReviewHelpfulness(ProductReview productReview, bool helpfulness)
        {
            ...
            var prh = _productReviewHelpfulnessRepository.Table.SingleOrDefault(h => h.ProductReviewId == productReview.Id && h.UserId == _workContext.CurrentUser.Id);
