...
namespace TVProgViewer.Services.Shipping
{
    ...
    public partial class ShippingService : IShippingService
    {
        ...
        {
            var key = TvProgShippingCachingDefaults.ShippingMethodsAllCacheKey.FillCacheKey(filterByCountryId);
        ...
        public virtual IList<Warehouse> GetAllWarehouses()
        {
            var query = from wh in _warehouseRepository.Table
                        orderby wh.Name
                        select wh;
            var warehouses = query.ToCachedList(TvProgShippingCachingDefaults.WarehousesAllCacheKey);
            return warehouses;
        }
            ...
            bool includeCheckoutAttributes = true, bool ignoreFreeShippedItems = false)
        {
            ...
            if (request.User is null || !includeCheckoutAttributes)
                return totalWeight;
            var checkoutAttributesXml = _genericAttributeService.GetAttribute<string>(request.User, TvProgUserDefaults.CheckoutAttributes, _storeContext.CurrentStore.Id);
            ...
            Address shippingAddress, int storeId, out bool shippingFromMultipleLocations)
        {
            ...
            foreach (var sci in cart)
            {
                ...
                else
                {
                    ...
                    //User
                    request.User = _userService.GetShoppingCartUser(cart);
            ...
            int storeId = 0)
        {
            ...
            var shippingRateComputationMethods = _shippingPluginManager
                .LoadActivePlugins(User, storeId, allowedShippingRateComputationMethodSystemName);
        ...
        public virtual GetPickupPointsResponse GetPickupPoints(int addressId, User User = null,
            string providerSystemName = null, int storeId = 0)
        {
            var result = new GetPickupPointsResponse();

            var pickupPointsProviders = _pickupPluginManager.LoadActivePlugins(User, storeId, providerSystemName);
