            ...
            foreach (var requirement in requirements)
            {
                ...
                else
                {
                    ...
                    var requirementRulePlugin = await _discountPluginManager
                        .LoadPluginBySystemNameAsync(requirement.DiscountRequirementRuleSystemName, user, (await _storeContext.GetCurrentStoreAsync()).Id);
                ...
                case DiscountLimitationType.NTimesPerUser:
                    {
                        ...
                        {
                            var usedTimes = (await GetAllDiscountUsageHistoryAsync(discount.Id, user.Id, null, 0, 1)).TotalCount;
                            if (usedTimes >= discount.LimitationTimes)
                            {
