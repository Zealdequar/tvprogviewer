    ...
    public partial class CommonModelFactory : ICommonModelFactory
    {
        ...
        public virtual async Task<HeaderLinksModel> PrepareHeaderLinksModelAsync()
        {
            ...
            if (unreadMessageCount > 0)
            {
                ...
                if (_forumSettings.ShowAlertForPM &&
                    !await _genericAttributeService.GetAttributeAsync<bool>(user, TvProgUserDefaults.NotifiedAboutNewPrivateMessagesAttribute, (await _storeContext.GetCurrentStoreAsync()).Id))
                {
                    await _genericAttributeService.SaveAttributeAsync(user, TvProgUserDefaults.NotifiedAboutNewPrivateMessagesAttribute, true, (await _storeContext.GetCurrentStoreAsync()).Id);
            ...
            var model = new HeaderLinksModel
            {
                ...
                ShoppingCartEnabled = false,// await _permissionService.AuthorizeAsync(StandardPermissionProvider.EnableShoppingCart),
                WishlistEnabled = false, //await _permissionService.AuthorizeAsync(StandardPermissionProvider.EnableWishlist),
            ...
            {
                model.ShoppingCartItems = (await _shoppingCartService.GetShoppingCartAsync(user, ShoppingCartType.ShoppingCart, (await _storeContext.GetCurrentStoreAsync()).Id))
                    .Sum(item => item.Quantity);

                model.WishlistItems = (await _shoppingCartService.GetShoppingCartAsync(user, ShoppingCartType.Wishlist, (await _storeContext.GetCurrentStoreAsync()).Id))
                    .Sum(item => item.Quantity);
            }
        ...
        public virtual async Task<FooterModel> PrepareFooterModelAsync()
        {
            ...
            var model = new FooterModel
            {
                ...
                HidePoweredByTvProgViewer = _storeInformationSettings.HidePoweredByTvProgViewer,
        ...
        public virtual async Task<ContactUsModel> PrepareContactUsModelAsync(ContactUsModel model, bool excludeProperties)
        {
            ...
            {
                model.Email = (await _workContext.GetCurrentUserAsync()).Email;
                model.FullName = await _userService.GetUserFullNameAsync(await _workContext.GetCurrentUserAsync());
            }
        ...
        public virtual async Task<SitemapModel> PrepareSitemapModelAsync(SitemapPageModel pageModel)
        {
            ...
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(TvProgModelCacheDefaults.SitemapPageModelKey,
                await _workContext.GetWorkingLanguageAsync(),
                _userService.GetUserRoleIdsAsync(await _workContext.GetCurrentUserAsync()),
        ...
        {
            var cacheKey = _staticCacheManager.PrepareKeyForDefaultCache(TvProgModelCacheDefaults.SitemapSeoModelKey, id,
                await _workContext.GetWorkingLanguageAsync(),
                _userService.GetUserRoleIdsAsync(await _workContext.GetCurrentUserAsync()),
                await _storeContext.GetCurrentStoreAsync());
