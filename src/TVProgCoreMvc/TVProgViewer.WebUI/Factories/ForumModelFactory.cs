    ...
    public partial class ForumModelFactory : IForumModelFactory
    {
        ...
        public virtual async Task<ForumPageModel> PrepareForumPageModelAsync(Forum forum, int page)
        {
            ...
            if (await _forumService.IsUserAllowedToSubscribeAsync(await _workContext.GetCurrentUserAsync()))
            {
                ...
                var forumSubscription = (await _forumService.GetAllSubscriptionsAsync((await _workContext.GetCurrentUserAsync()).Id, forum.Id, 0, 0, 1)).FirstOrDefault();
            ...
            model.IsUserAllowedToSubscribe = await _forumService.IsUserAllowedToSubscribeAsync(await _workContext.GetCurrentUserAsync());
        ...
        public virtual async Task<ForumTopicPageModel> PrepareForumTopicPageModelAsync(ForumTopic forumTopic, int page)
        {
            ...
            var model = new ForumTopicPageModel
            {
                ...
                IsUserAllowedToEditTopic = await _forumService.IsUserAllowedToEditTopicAsync(await _workContext.GetCurrentUserAsync(), forumTopic),
                IsUserAllowedToDeleteTopic = await _forumService.IsUserAllowedToDeleteTopicAsync(await _workContext.GetCurrentUserAsync(), forumTopic),
                IsUserAllowedToMoveTopic = await _forumService.IsUserAllowedToMoveTopicAsync(await _workContext.GetCurrentUserAsync(), forumTopic),
                IsUserAllowedToSubscribe = await _forumService.IsUserAllowedToSubscribeAsync(await _workContext.GetCurrentUserAsync())
            };
            ...
            if (model.IsUserAllowedToSubscribe)
            {
                ...
                var forumTopicSubscription = (await _forumService.GetAllSubscriptionsAsync((await _workContext.GetCurrentUserAsync()).Id, 0, forumTopic.Id, 0, 1)).FirstOrDefault();
            ...
            foreach (var post in posts)
            {
                ...
                var userIsModerator = userIsGuest ? false : await _userService.IsForumModeratorAsync(user);
                ...
                var forumPostModel = new ForumPostModel
                {
                    ...
                    IsCurrentUserAllowedToEditPost = await _forumService.IsUserAllowedToEditPostAsync(await _workContext.GetCurrentUserAsync(), post),
                    IsCurrentUserAllowedToDeletePost = await _forumService.IsUserAllowedToDeletePostAsync(await _workContext.GetCurrentUserAsync(), post),
                    UserId = post.UserId,
                    ...
                    ForumPostCount = await _genericAttributeService.GetAttributeAsync<User, int>(post.UserId, TvProgUserDefaults.ForumPostCountAttribute),
                    ...
                    FormattedSignature = _forumService.FormatForumSignatureText(await _genericAttributeService.GetAttributeAsync<User, string>(post.UserId, TvProgUserDefaults.SignatureAttribute)),
                };
                //created on string
                ...
                if (_userSettings.AllowUsersToUploadAvatars)
                {
                    forumPostModel.UserAvatarUrl = await _pictureService.GetPictureUrlAsync(
                        await _genericAttributeService.GetAttributeAsync<User, int>(post.UserId, TvProgUserDefaults.AvatarPictureIdAttribute),
                ...
                {
                    var countryId = await _genericAttributeService.GetAttributeAsync<User, int>(post.UserId, TvProgUserDefaults.CountryIdAttribute);
                ...
                if (_forumSettings.AllowPostVoting)
                {
                    ...
                    var postVote = await _forumService.GetPostVoteAsync(post.Id, await _workContext.GetCurrentUserAsync());
                    if (postVote != null)
        ...
        public virtual async Task PrepareTopicCreateModelAsync(Forum forum, EditForumTopicModel model)
        {
            ...
            model.IsUserAllowedToSetTopicPriority = await _forumService.IsUserAllowedToSetTopicPriorityAsync(await _workContext.GetCurrentUserAsync());
        ...
        public virtual async Task PrepareTopicEditModelAsync(ForumTopic forumTopic, EditForumTopicModel model, bool excludeProperties)
        {
            ...
            if (!excludeProperties)
            {
                ...
                if (model.IsUserAllowedToSubscribe)
                {
                    var forumSubscription = (await _forumService.GetAllSubscriptionsAsync((await _workContext.GetCurrentUserAsync()).Id, 0, forumTopic.Id, 0, 1)).FirstOrDefault();
                    ...
                    if (quotePost != null && quotePost.TopicId == forumTopic.Id)
                    {
                        ...
                        switch (_forumSettings.ForumEditor)
                        {
                            ...
                            case EditorType.BBCodeEditor:
                                text = $"[quote={await _userService.FormatUsernameAsync(user)}]{BBCodeHelper.RemoveQuotes(quotePostText)}[/quote]";
                                break;
                        }
                        model.Text = text;
                    }
            ...
            string within, string limitDays, int page)
        {
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(searchterms))
                {
                    ...
                    {
                        throw new TvProgException(string.Format(await _localizationService.GetResourceAsync("Forum.SearchTermMinimumLengthIsNCharacters"),
