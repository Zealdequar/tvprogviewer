...
namespace TVProgViewer.WebUI.Factories
{
    ...
    public partial class ForumModelFactory : IForumModelFactory
    {
        ...
        public virtual ForumTopicRowModel PrepareForumTopicRowModel(ForumTopic topic)
        {
            ...
            var user = _userService.GetUserById(topic.UserId);
            ...
            var topicModel = new ForumTopicRowModel
            {
                ...
                UserId = topic.UserId,
                AllowViewingProfiles = _userSettings.AllowViewingProfiles && !_userService.IsGuest(user),
        ...
        public virtual ForumTopicPageModel PrepareForumTopicPageModel(ForumTopic forumTopic, int page)
        {
            ...
            foreach (var post in posts)
            {
                ...
                var userIsGuest = _userService.IsGuest(user);
                var userIsModerator = userIsGuest ? false : _userService.IsForumModerator(user);
                ...
                var forumPostModel = new ForumPostModel
                {
                    ...
                    ForumPostCount = _genericAttributeService.GetAttribute<User, int>(post.UserId, TvProgUserDefaults.ForumPostCountAttribute),
                    ...
                    FormattedSignature = _forumService.FormatForumSignatureText(_genericAttributeService.GetAttribute<User, string>(post.UserId, TvProgUserDefaults.SignatureAttribute)),
                };
                //created on string
                ...
                if (_userSettings.AllowUsersToUploadAvatars)
                {
                    forumPostModel.UserAvatarUrl = _pictureService.GetPictureUrl(
                        _genericAttributeService.GetAttribute<User, int>(post.UserId, TvProgUserDefaults.AvatarPictureIdAttribute),
                ...
                {
                    var countryId = _genericAttributeService.GetAttribute<User, int>(post.UserId, TvProgUserDefaults.CountryIdAttribute);
        ...
        public virtual EditForumPostModel PreparePostCreateModel(ForumTopic forumTopic, int? quote, bool excludeProperties)
        {
            ...
            if (!excludeProperties)
            {
                ...
                if (quote.HasValue)
                {
                    ...
                    var user = _userService.GetUserById(quotePost.UserId);
                    ...
                    if (quotePost != null && quotePost.TopicId == forumTopic.Id)
                    {
                        ...
                        switch (_forumSettings.ForumEditor)
                        {
                            case EditorType.SimpleTextBox:
                                text = $"{_userService.FormatUsername(user)}:\n{quotePostText}\n";
                                break;
                            case EditorType.BBCodeEditor:
                                text = $"[quote={_userService.FormatUsername(user)}]{BBCodeHelper.RemoveQuotes(quotePostText)}[/quote]";
                                break;
                        }
                        model.Text = text;
                    }
            ...
            string within, string limitDays, int page)
        {
            ...
            try
            {
                if (!string.IsNullOrWhiteSpace(searchterms))
                {
                    ...
                    {
                        throw new TvProgException(string.Format(_localizationService.GetResource("Forum.SearchTermMinimumLengthIsNCharacters"),
        ...
        public virtual LastPostModel PrepareLastPostModel(ForumPost forumPost, bool showTopic)
        {
            ...
            model.AllowViewingProfiles = _userSettings.AllowViewingProfiles && !_userService.IsGuest(user);
            model.UserName = _userService.FormatUsername(user);
            //created on string
            var languageCode = _workContext.WorkingLanguage.LanguageCulture;
            if (_forumSettings.RelativeDateTimeFormattingEnabled)
            {
