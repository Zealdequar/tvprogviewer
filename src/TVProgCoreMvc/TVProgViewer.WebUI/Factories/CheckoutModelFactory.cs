...
namespace TVProgViewer.WebUI.Factories
{
    public partial class CheckoutModelFactory : ICheckoutModelFactory
    {
        ...
        protected virtual async Task<CheckoutPickupPointsModel> PrepareCheckoutPickupPointsModelAsync(IList<ShoppingCartItem> cart)
        {
            ...
            var pickupPointProviders = await _pickupPluginManager.LoadActivePluginsAsync(await _workContext.GetCurrentUserAsync(), (await _storeContext.GetCurrentStoreAsync()).Id);
            if (pickupPointProviders.Any())
            {
                var languageId = (await _workContext.GetWorkingLanguageAsync()).Id;
                var pickupPointsResponse = await _shippingService.GetPickupPointsAsync((await _workContext.GetCurrentUserAsync()).BillingAddressId ?? 0,
                ...
                if (pickupPointsResponse.Success)
                    model.PickupPoints = await pickupPointsResponse.PickupPoints.SelectAwait(async point =>
                    {
                        ...
                        if (amount > 0)
                        {
                            (amount, _) = await _taxService.GetShippingPriceAsync(amount, await _workContext.GetCurrentUserAsync());
                        ...
                        var (rateBase, _) = await _taxService.GetShippingPriceAsync(shippingTotal, await _workContext.GetCurrentUserAsync());
            ...
            var shippingProviders = await _shippingPluginManager.LoadActivePluginsAsync(await _workContext.GetCurrentUserAsync(), (await _storeContext.GetCurrentStoreAsync()).Id);
            ...
            string overrideAttributesXml = "")
        {
            var model = new CheckoutBillingAddressModel
            {
                ...
                //allow users to enter (choose) a shipping address if "Disable Billing address step" setting is enabled
            ...
            await _addressModelFactory.PrepareAddressModelAsync(model.BillingNewAddress,
                ...
                prePopulateWithUserFields: prePopulateNewAddressWithUserFields,
        ...
        public virtual async Task<CheckoutShippingAddressModel> PrepareShippingAddressModelAsync(IList<ShoppingCartItem> cart,
            int? selectedCountryId = null, bool prePopulateNewAddressWithUserFields = false, string overrideAttributesXml = "")
        {
        ...
        public virtual async Task<CheckoutShippingMethodModel> PrepareShippingMethodModelAsync(IList<ShoppingCartItem> cart, Address shippingAddress)
        {
            ...
            var getShippingOptionResponse = await _shippingService.GetShippingOptionsAsync(cart, shippingAddress, await _workContext.GetCurrentUserAsync(), storeId: (await _storeContext.GetCurrentStoreAsync()).Id);
            if (getShippingOptionResponse.Success)
            {
                ...
                var selectedShippingOption = await _genericAttributeService.GetAttributeAsync<ShippingOption>(await _workContext.GetCurrentUserAsync(),
                ...
                if (_shippingSettings.NotifyUserAboutShippingFromMultipleLocations)
                {
                    model.NotifyUserAboutShippingFromMultipleLocations = getShippingOptionResponse.ShippingFromMultipleLocations;
                }
            }
            else
            {
        ...
        public virtual async Task<CheckoutPaymentMethodModel> PreparePaymentMethodModelAsync(IList<ShoppingCartItem> cart, int filterByCountryId)
        {
            ...
            {
                var rewardPointsBalance = await _rewardPointService.GetRewardPointsBalanceAsync((await _workContext.GetCurrentUserAsync()).Id, (await _storeContext.GetCurrentStoreAsync()).Id);
            ...
            var paymentMethods = await (await _paymentPluginManager
                .LoadActivePluginsAsyncAsync(await _workContext.GetCurrentUserAsync(), (await _storeContext.GetCurrentStoreAsync()).Id, filterByCountryId))
            ...
            foreach (var pm in paymentMethods)
            {
                ...
                var (rateBase, _) = await _taxService.GetPaymentMethodAdditionalFeeAsync(paymentMethodAdditionalFee, await _workContext.GetCurrentUserAsync());
                var rate = await _currencyService.ConvertFromPrimaryStoreCurrencyAsync(rateBase, await _workContext.GetWorkingCurrencyAsync());
            ...
            var selectedPaymentMethodSystemName = await _genericAttributeService.GetAttributeAsync<string>(await _workContext.GetCurrentUserAsync(),
        ...
        public virtual async Task<OnePageCheckoutModel> PrepareOnePageCheckoutModelAsync(IList<ShoppingCartItem> cart)
        {
            ...
            var model = new OnePageCheckoutModel
            {
                ...
                DisableBillingAddressCheckoutStep = _orderSettings.DisableBillingAddressCheckoutStep && (await _userService.GetAddressesByUserIdAsync((await _workContext.GetCurrentUserAsync()).Id)).Any(),
                BillingAddress = await PrepareBillingAddressModelAsync(cart, prePopulateNewAddressWithUserFields: true)
            };
            return model;
        }
