    ...
    public partial class CheckoutModelFactory : ICheckoutModelFactory
    {
            ...
            bool prePopulateNewAddressWithUserFields = false, string overrideAttributesXml = "")
        {
            ...
            if (model.AllowPickupInStore)
            {
                ...
                var pickupPointProviders = _pickupPluginManager.LoadActivePlugins(_workContext.CurrentUser, _storeContext.CurrentStore.Id);
                if (pickupPointProviders.Any())
                {
                    var languageId = _workContext.WorkingLanguage.Id;
                    var pickupPointsResponse = _shippingService.GetPickupPoints(_workContext.CurrentUser.BillingAddressId ?? 0,
                        _workContext.CurrentUser, storeId: _storeContext.CurrentStore.Id);
                    if (pickupPointsResponse.Success)
                        model.PickupPoints = pickupPointsResponse.PickupPoints.Select(point =>
                        {
                            ...
                            {
                                var amount = _taxService.GetShippingPrice(point.PickupFee, _workContext.CurrentUser);
                ...
                var shippingProviders = _shippingPluginManager.LoadActivePlugins(_workContext.CurrentUser, _storeContext.CurrentStore.Id);
        ...
        public virtual CheckoutShippingMethodModel PrepareShippingMethodModel(IList<ShoppingCartItem> cart, Address shippingAddress)
        {
            ...
            var getShippingOptionResponse = _shippingService.GetShippingOptions(cart, shippingAddress, _workContext.CurrentUser, storeId: _storeContext.CurrentStore.Id);
            if (getShippingOptionResponse.Success)
            {
                ...
                foreach (var shippingOption in getShippingOptionResponse.ShippingOptions)
                {
                    ...
                    var rateBase = _taxService.GetShippingPrice(shippingTotal, _workContext.CurrentUser);
                ...
                var selectedShippingOption = _genericAttributeService.GetAttribute<ShippingOption>(_workContext.CurrentUser,
        ...
        public virtual CheckoutPaymentMethodModel PreparePaymentMethodModel(IList<ShoppingCartItem> cart, int filterByCountryId)
        {
            ...
            {
                var rewardPointsBalance = _rewardPointService.GetRewardPointsBalance(_workContext.CurrentUser.Id, _storeContext.CurrentStore.Id);
            ...
            var paymentMethods = _paymentPluginManager
                .LoadActivePlugins(_workContext.CurrentUser, _storeContext.CurrentStore.Id, filterByCountryId)
            ...
            foreach (var pm in paymentMethods)
            {
                ...
                var rateBase = _taxService.GetPaymentMethodAdditionalFee(paymentMethodAdditionalFee, _workContext.CurrentUser);
                var rate = _currencyService.ConvertFromPrimaryStoreCurrency(rateBase, _workContext.WorkingCurrency);
            ...
            var selectedPaymentMethodSystemName = _genericAttributeService.GetAttribute<string>(_workContext.CurrentUser,
        ...
        public virtual OnePageCheckoutModel PrepareOnePageCheckoutModel(IList<ShoppingCartItem> cart)
        {
            ...
            var model = new OnePageCheckoutModel
            {
                ...
                DisableBillingAddressCheckoutStep = _orderSettings.DisableBillingAddressCheckoutStep && _userService.GetAddressesByUserId(_workContext.CurrentUser.Id).Any(),
                BillingAddress = PrepareBillingAddressModel(cart, prePopulateNewAddressWithUserFields: true)
            };
            return model;
        }
