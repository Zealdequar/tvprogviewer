...
namespace TVProgViewer.WebUI.Areas.Admin.Controllers
{
    public partial class UserController : BaseAdminController
    {
        ...
        public virtual async Task<IActionResult> Create(UserModel model, bool continueEditing, IFormCollection form)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (_userSettings.DateOfBirthEnabled)
                    await _genericAttributeService.SaveAttributeAsync(user, TvProgUserDefaults.DateOfBirthAttribute, model.DateOfBirth);
        ...
        public virtual async Task<IActionResult> Impersonate(int id)
        {
            ...
            if (!await _userService.IsAdminAsync(await _workContext.GetCurrentUserAsync()) && await _userService.IsAdminAsync(user))
            {
            ...
            await _userActivityService.InsertActivityAsync(user, "Impersonation.Started",
                string.Format(await _localizationService.GetResourceAsync("ActivityLog.Impersonation.Started.User"), (await _workContext.GetCurrentUserAsync()).Email, (await _workContext.GetCurrentUserAsync()).Id), await _workContext.GetCurrentUserAsync());

            //ensure login is not required
            user.RequireReLogin = false;
            await _userService.UpdateUserAsync(user);
            await _genericAttributeService.SaveAttributeAsync<int?>(await _workContext.GetCurrentUserAsync(), TvProgUserDefaults.ImpersonatedUserIdAttribute, user.Id);
        ...
        public virtual async Task<IActionResult> GdprExport(int id)
        {
            ...
            try
            {
                ...
                //export
                //export
                var bytes = await _exportManager.ExportUserGdprInfoToXlsxAsync(user, (await _storeContext.GetCurrentStoreAsync()).Id);
