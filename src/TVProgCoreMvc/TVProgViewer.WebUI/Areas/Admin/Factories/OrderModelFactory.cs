    ...
    public partial class OrderModelFactory : IOrderModelFactory
    {
                ...
                if (attribute.ShouldHaveValues())
                {
                    ...
                    foreach (var attributeValue in attributeValues)
                    {
                        ...
                        var priceAdjustment = _taxService.GetProductPrice(product,
                            _priceCalculationService.GetProductAttributeValuePriceAdjustment(product, attributeValue, user), out _);
        ...
        public virtual OrderModel PrepareOrderModel(OrderModel model, Order order, bool excludeProperties = false)
        {
            if (order != null)
            {
                ...
                model.UserInfo = _userService.IsRegistered(user) ? user.Email : _localizationService.GetResource("Admin.Users.Guest");
        ...
        public virtual AddProductToOrderModel PrepareAddProductToOrderModel(AddProductToOrderModel model, Order order, Product product)
        {
            ...
            var presetQty = 1;
            var presetPrice = _priceCalculationService.GetFinalPrice(product, user, decimal.Zero, true, presetQty);
            var presetPriceInclTax = _taxService.GetProductPrice(product, presetPrice, true, user, out _);
            var presetPriceExclTax = _taxService.GetProductPrice(product, presetPrice, false, user, out _);
