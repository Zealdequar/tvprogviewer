...
namespace TVProgViewer.WebUI.Concrete
{
    public class EFChannelRepository: BaseEFRepository, IChannelsRepository
    {
        ...
        internal Task<List<SystemChannel>> GetUserInSystemChannels(long uid, int progProviderID, int typeProgID)
        {
            return Task<List<SystemChannel>>.Factory.StartNew(() => { return TvProgService.GetUserInSystemChannels(uid, progProviderID, typeProgID); });
        }
