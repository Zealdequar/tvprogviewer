...
namespace TvProgViewer.Tests.TvProgViewer.Core.Tests.Caching
{
    [TestFixture]
    public class LockerTests : BaseTvProgTest
    {
        ...
        private static async Task TestLockerAsync(ILocker locker)
        {
            ...
            async Task action()
            {
                var res = await locker.PerformActionWithLockAsync(key, expiration,
                    () =>
                    {
                        Assert.Fail("Действие выполняется");
        ...
        private static async Task TestHeartbeatAsync(ILocker locker)
        {
            ...
            async Task action(CancellationToken _)
            {
                await locker.RunWithHeartbeatAsync(
                    ...
                    _ =>
                    {
                        Assert.Fail("Действие в процессе");
                        return Task.CompletedTask;
                    });
        ...
        {
            throw new ApplicationException("Предполагается, что это действие завершится неудачей");
        }
    }
}