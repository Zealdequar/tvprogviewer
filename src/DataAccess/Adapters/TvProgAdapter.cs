                    ...
                    case 2:
                        if (dateTime == minDate)
                        {
                            ...
                            var sp2 = (from pr in dataContext.Programmes.AsNoTracking()
                                               ...
                                               ).AsNoTracking().Select(mapper.Map<SystemProgramme>);
                            ...
                            count = sp3.Count();
                            if (!string.IsNullOrWhiteSpace(sidx))
                                systemProgramme = LinqExtensions.OrderBy(sp3.AsQueryable(), sidx, sord).Skip((page - 1) * rows).Take(rows).Select(mapper.Map<SystemProgramme>).ToList<SystemProgramme>();
                            else systemProgramme = sp3.Skip((page - 1) * rows).Take(rows).Select(mapper.Map<SystemProgramme>).ToList<SystemProgramme>();
                    ...
                    case 1:
                        var sp = (from pr in dataContext.Programmes.AsNoTracking()
                                           ...
                                           select new
                                           {
                                               ...
                                               Category = pr.Category,
                                               Remain = (int)(DbFunctions.DiffSeconds(pr.TsStopMO, dateTime) * 1.0 / (DbFunctions.DiffSeconds(pr.TsStopMO, pr.TsStartMO) * 1.0) * 100.0),
                                           }).ToList().DistinctBy(x => x.ProgrammesID).OrderBy(x => x.OrderCol);
                        count = sp.Count();
                    ...
                    case 2:
                        if (dateTime == minDate)
                        {
                            ...
                            var sp2 = (from pr in dataContext.Programmes.AsNoTracking()
                                               ...
                                               }).AsNoTracking().OrderBy(x => x.OrderCol).Select(mapper.Map<SystemProgramme>);
                            var sp3 = (from pr in sp2
                                               join prin in stopAfter on pr.CID equals prin.CID
                                               where pr.TsStartMO <= prin.TsStopMOAfter && prin.TsStopMOAfter < pr.TsStopMO
                                               select new SystemProgramme()
                        ...
                        else if (dateTime > minDate)
                        {
                            var sp4 = (from pr in dataContext.Programmes.AsNoTracking()
                                               ...
                                               ).AsNoTracking().OrderBy(o => o.OrderCol);
                            count = sp4.Count();
                            if (!string.IsNullOrWhiteSpace(sidx))
                                systemProgramme = LinqExtensions.OrderBy(sp4.AsQueryable(), sidx, sord).Take(rows).Skip((page - 1) * rows).Select(mapper.Map<SystemProgramme>).ToList<SystemProgramme>();
                            else systemProgramme = sp4.Skip((page - 1) * rows).Take(rows).Select(mapper.Map<SystemProgramme>).ToList<SystemProgramme>();
