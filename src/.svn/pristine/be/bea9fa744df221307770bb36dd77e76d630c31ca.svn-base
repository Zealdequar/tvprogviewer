...
#if DSHOWNET
using DShowNET;
using DirectX.Capture.Properties;
...
namespace DirectX.Capture
{
	...
	public class PropertyPageCollection : CollectionBase, IDisposable
	{
			...
			SourceCollection videoSources, SourceCollection audioSources)
		{
			...
			addIfSupported( videoDeviceFilter, Resources.VideoCaptureDeviceText );
			...
			addIfSupported( filter, Resources.VideoCapturePinText );
			...
			addIfSupported( filter, Resources.VideoPreviewPinText );
			...
			for ( int c = 0; c < videoSources.Count; c++ )
			{
				...
				if ( s != null )
				{
					if ( crossbars.IndexOf( s.Crossbar ) < 0 )
					{
						...
						if ( addIfSupported( s.Crossbar, Resources.VideoCrossbarText + ( num==1 ? "" : num.ToString() ) ) )
							num++;
					}
				}
			}
			...
			addIfSupported( videoCompressorFilter, Resources.VideoCompressorText );
			...
			addIfSupported( filter, Resources.TVTunerText );
...
//#if NEWCODE
			...
			addIfSupported( filter, Resources.TVAudioText );
//#endif
			...
			{
				VfwCompressorPropertyPage page = new VfwCompressorPropertyPage( Resources.VideoCompressorText, compressDialog );
				InnerList.Add( page );
			}

			// 8. the audio capture filter
			addIfSupported( audioDeviceFilter, Resources.AudioCaptureDeviceText );
			...
			addIfSupported( filter, Resources.AudioCaptuePinText);
			...
			addIfSupported( filter, Resources.AudioPreviewPinText);
			...
			for ( int c = 0; c < audioSources.Count; c++ )
			{
				...
				if ( s != null )
				{
					...
					{
						crossbars.Add( s.Crossbar );
						if ( addIfSupported( s.Crossbar, Resources.AudioCrossbarText + ( num==1 ? "" : num.ToString() ) ) )
							num++;
					}
				}
			}
			...
			addIfSupported( audioCompressorFilter, Resources.AudioCompressorText );
