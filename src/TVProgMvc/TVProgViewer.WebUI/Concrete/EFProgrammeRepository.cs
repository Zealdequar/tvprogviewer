...
namespace TVProgViewer.WebUI.Concrete
{
    public class EFProgrammeRepository : BaseEFRepository, IProgrammesRepository
    {
        ...
        /// <param name="dateTimeOffset">Время</param>
        /// <param name="category">Категория</param>
        public Task<KeyValuePair<int, SystemProgramme[]>> GetUserProgrammesAtNowAsyncList(long uid, int typeProgID, DateTimeOffset dateTimeOffset, string category, string sidx, string sord, int page, int rows)
        {
            return Task<KeyValuePair<int, SystemProgramme[]>>.Factory.StartNew(() => { return TvProgService.GetUserProgrammeList(uid, typeProgID, dateTimeOffset, 1, category, sidx, sord, page, rows); });
        }
        ...
        public Task<KeyValuePair<int, SystemProgramme[]>> GetUserProgrammesAtNextAsyncList(long uid, int typeProgID, DateTimeOffset dateTimeOffset, string category,
            string sidx, string sord, int page, int rows)
        {
            return Task<KeyValuePair<int, SystemProgramme[]>>.Factory.StartNew(() => { return TvProgService.GetUserProgrammeList(uid, typeProgID, dateTimeOffset, 2, category, sidx, sord, page, rows); });
        }
        ...
        public Task<KeyValuePair<int,SystemProgramme[]>> SearchProgramme(int typeProgID, string findTitle, string sidx, string sord, int page, int rows)
        {
            return Task<KeyValuePair<int, SystemProgramme[]>>.Factory.StartNew(() => { return TvProgService.SearchProgramme(typeProgID, findTitle, sidx, sord, page, rows); });
        }
