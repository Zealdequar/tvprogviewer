...
namespace TVProgViewer.WebUI.Controllers
{
    ...
    public class ProgrammeController : Controller
    {
        ...
        public async Task<ActionResult> GetSystemProgrammeAtNow(int progType, string category, string sidx, string sord, int page, int rows)
        {
            ...
            {
               result = await repository.GetUserProgrammesAtNowAsyncList(UserId.Value, progType, DateTimeOffset.Now, (category != "null") ? category : null, 
                     sidx, sord, page, rows);
            }
            else
            {
               result = await repository.GetSystemProgrammesAtNowAsyncList(progType, DateTimeOffset.Now, (category != "null") ? category : null,
                     sidx, sord, page, rows);
            }
        ...
        public async Task<ActionResult> GetSystemProgrammeAtNext(int progType, string category, string sidx, string sord, int page, int rows)
        {
            ...
            if (User.Identity.IsAuthenticated && UserId != null)
            {
                result = await repository.GetUserProgrammesAtNextAsyncList(UserId.Value, progType, new DateTimeOffset(new DateTime(1800, 1, 1)), (category != "null") ? category : null,
                    sidx, sord, page, rows);
            }
            else
            {
                result = await repository.GetSystemProgrammesAtNextAsyncList(progType,
                   new DateTimeOffset(new DateTime(1800, 1, 1)), (category != "null") ? category : null, sidx, sord, page, rows);
            }
            int pageIndex = page - 1;
            int pageSize = rows;
            int totalRecords = result.Key;
        ...
        public async Task<ActionResult> SearchProgramme(int progType, string findTitle, string sidx, string sord, int page, int rows)
        {
            if (User.Identity.IsAuthenticated && UserId != null)
            {
                return Json(await repository.SearchUserProgramme(UserId.Value, progType, findTitle), JsonRequestBehavior.AllowGet);
            }
            KeyValuePair<int, SystemProgramme[]> result = await repository.SearchProgramme(progType, findTitle, sidx, sord, page, rows);
            int pageIndex = page - 1;
            int pageSize = rows;
            int totalRecords = result.Key;
            int totalPages = (int)Math.Ceiling((float)totalRecords / (float)pageSize);
