...
namespace TvProgViewer.WebUI.Factories
{
    public partial class CheckoutModelFactory : ICheckoutModelFactory
    {
        ...
        protected virtual async Task<CheckoutPickupPointsModel> PrepareCheckoutPickupPointsModelAsync(IList<ShoppingCartItem> cart)
        {
            ...
            var pickupPointProviders = await _pickupPluginManager.LoadActivePluginsAsync(user, store.Id);
            if (pickupPointProviders.Any())
            {
                ...
                if (pickupPointsResponse.Success)
                    model.PickupPoints = await pickupPointsResponse.PickupPoints.SelectAwait(async point =>
                    {
                        ...
                        if (amount > 0)
                        {
                            (amount, _) = await _taxService.GetShippingPriceAsync(amount, user);
                        ...
                        var (rateBase, _) = await _taxService.GetShippingPriceAsync(shippingTotal, user);
            ...
            var shippingProviders = await _shippingPluginManager.LoadActivePluginsAsync(user, store.Id);
            ...
            string overrideAttributesXml = "")
        {
            ...
            var user = await _workContext.GetCurrentUserAsync();
            if (await _userService.IsGuestAsync(user) && _taxSettings.EuVatEnabled)
            {
            ...
            int? selectedCountryId = null, bool prePopulateNewAddressWithUserFields = false, string overrideAttributesXml = "")
        {
            ...
            model.SelectedBillingAddress = user.BillingAddressId ?? 0;
        ...
        public virtual async Task<CheckoutShippingMethodModel> PrepareShippingMethodModelAsync(IList<ShoppingCartItem> cart, Address shippingAddress)
        {
            ...
            var user = await _workContext.GetCurrentUserAsync();
            var store = await _storeContext.GetCurrentStoreAsync();
            var getShippingOptionResponse = await _shippingService.GetShippingOptionsAsync(cart, shippingAddress, user, storeId: store.Id);
            if (getShippingOptionResponse.Success)
            {
                ...
                var selectedShippingOption = await _genericAttributeService.GetAttributeAsync<ShippingOption>(user,
        ...
        public virtual async Task<CheckoutPaymentMethodModel> PreparePaymentMethodModelAsync(IList<ShoppingCartItem> cart, int filterByCountryId)
        {
            ...
            if (_rewardPointsSettings.Enabled && !await _shoppingCartService.ShoppingCartIsRecurringAsync(cart))
            {
                ...
                if (shoppingCartTotal.redeemedRewardPoints > 0)
                {
                    ...
                    model.RewardPointsBalance = await _rewardPointService.GetRewardPointsBalanceAsync(user.Id, store.Id);
            ...
            var paymentMethods = await (await _paymentPluginManager
                .LoadActivePluginsAsync(user, store.Id, filterByCountryId))
            ...
            foreach (var pm in paymentMethods)
            {
                ...
                var (rateBase, _) = await _taxService.GetPaymentMethodAdditionalFeeAsync(paymentMethodAdditionalFee, user);
                var rate = await _currencyService.ConvertFromPrimaryStoreCurrencyAsync(rateBase, await _workContext.GetWorkingCurrencyAsync());
            ...
            var selectedPaymentMethodSystemName = await _genericAttributeService.GetAttributeAsync<string>(user,
        ...
        public virtual async Task<CheckoutConfirmModel> PrepareConfirmOrderModelAsync(IList<ShoppingCartItem> cart)
        {
            var model = new CheckoutConfirmModel
            {
                ...
                DisplayCaptcha = await _userService.IsGuestAsync(await _userService.GetShoppingCartUserAsync(cart))
        ...
        public virtual async Task<OnePageCheckoutModel> PrepareOnePageCheckoutModelAsync(IList<ShoppingCartItem> cart)
        {
            ...
            var model = new OnePageCheckoutModel
            {
                ...
                DisableBillingAddressCheckoutStep = _orderSettings.DisableBillingAddressCheckoutStep && (await _userService.GetAddressesByUserIdAsync(user.Id)).Any(),
