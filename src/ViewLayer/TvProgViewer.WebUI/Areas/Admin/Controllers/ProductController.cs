...
namespace TvProgViewer.WebUI.Areas.Admin.Controllers
{
    public partial class ProductController : BaseAdminController
    {
        ...
        {
            product.SubjectToAcl = model.SelectedUserRoleIds.Any();
            ...
            foreach (var userRole in allUserRoles)
            {
                if (model.SelectedUserRoleIds.Contains(userRole.Id))
                {
                    ...
                    if (!existingAclRecords.Any(acl => acl.UserRoleId == userRole.Id))
                        await _aclService.InsertAclRecordAsync(product, userRole.Id);
                }
                else
                {
                    //remove role
        ...
        public virtual async Task<IActionResult> Create(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //activity log
                await _userActivityService.InsertActivityAsync("AddNewProduct",
        ...
        public virtual async Task<IActionResult> Edit(ProductModel model, bool continueEditing)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                //activity log
                await _userActivityService.InsertActivityAsync("EditProduct",
        ...
        public virtual async Task<IActionResult> Delete(int id)
        {
            ...
            //activity log
            await _userActivityService.InsertActivityAsync("DeleteProduct",
                    ...
                    if (mappings?.Any(m => m.Id == productAttributeMapping.Id) == true)
                    {
                        _notificationService.ErrorNotification(
                            string.Format(await _localizationService.GetResourceAsync("Admin.Catalog.Products.ProductAttributes.Attributes.AlreadyExistsInCombination"),
                                await _productAttributeFormatter.FormatAttributesAsync(product, combination.AttributesXml, await _workContext.GetCurrentUserAsync(), await _storeContext.GetCurrentStoreAsync(), ", ")));
        ...
        public virtual async Task<IActionResult> ProductAttributeValueEditPopup(ProductAttributeValueModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                productAttributeValue.Quantity = model.UserEntersQty ? 1 : model.Quantity;
