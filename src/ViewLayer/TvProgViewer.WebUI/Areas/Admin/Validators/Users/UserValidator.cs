...
{
    public partial class UserValidator : BaseTvProgValidator<UserModel>
    {
            ...
            IStateProvinceService stateProvinceService)
        {
            ...
            {
                RuleFor(x => x.SmartPhone)
                    .NotEmpty()
                    .WithMessageAwait(localizationService.GetResourceAsync("Admin.Users.Users.Fields.SmartPhone.Required"))
                    //only for registered users
                    .WhenAwait(async x => await IsRegisteredUserRoleCheckedAsync(x, userService));
            }
        ...
        private async Task<bool> IsRegisteredUserRoleCheckedAsync(UserModel model, IUserService userService)
        {
            ...
            var isInRegisteredRole = newUserRoles.FirstOrDefault(cr => cr.SystemName == TvProgUserDefaults.RegisteredRoleName) != null;
